	...
	public function login(Request $req) {
		...
		if(isset($xen_user->user)){
			...
			if($x_user->setUserById($xen_user->user)){
				//Borrar las salas activas del usuario
				$rooms = RoomUser::where('user_id', $xen_user->user->user_id)->get();
				foreach($rooms as $r){
					$r->delete();
				}
				return response()
						->json($xen_user)
						->cookie('xf_user', $xen_user->user->user_id, 5); //el número es minutos de vida de la cookie
			}else{
